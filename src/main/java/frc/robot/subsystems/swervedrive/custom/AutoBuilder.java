...
public class AutoBuilder {
    ...
    private static QuadFunction<Translation2d, Function<PathPlannerTrajectory.State, Rotation2d>, PathConstraints, Double, Command> pathfindToPoseFunctionalRotationCommandBuilder;
     ...
     * @param resetPose
     *                                    a consumer for resetting the robot's pose
     * @param robotRelativeSpeedsSupplier
     *                                    a supplier for the robot's current robot
     *                                    relative chassis
     *                                    speeds
     * @param robotRelativeOutput
     *                                    a consumer for setting the robot's
     *                                    robot-relative chassis speeds
     * @param config
     *                                    {@link com.pathplanner.lib.util.HolonomicPathFollowerConfig}
     *                                    for
     *                                    configuring the
     *                                    path following commands
     * @param shouldFlipPath
     *                                    Supplier that determines if paths should
     *                                    be flipped to the other
     *                                    side of
    ...
    public static void configureHolonomic(
        ...
        AutoBuilder.pathFollowingCommandBuilder = (path) -> new FollowPathHolonomic(
                path,
                poseSupplier,
        ...
        AutoBuilder.pathfindToPoseCommandBuilder = (pose, constraints, goalEndVel,
                rotationDelayDistance) -> new com.pathplanner.lib.commands.PathfindHolonomic(
                        pose,
                        constraints,
                        goalEndVel,
                        poseSupplier,
                        ...
                        driveSubsystem);
        AutoBuilder.pathfindToPoseFunctionalRotationCommandBuilder = (position, rotationFunction, constraints,
                goalEndVel) -> new PathfindHolonomic(
                        position,
                        rotationFunction,
                        constraints,
                        goalEndVel,
                        ...
                        config,
                        driveSubsystem);
        AutoBuilder.pathfindThenFollowPathCommandBuilder = (path, constraints,
                rotationDelayDistance) -> new PathfindThenFollowPathHolonomic(
                        path,
                        constraints,
                        poseSupplier,
                        robotRelativeSpeedsSupplier,
     ...
     *
     * @return true if the AutoBuilder has been configured for pathfinding, false
     *         otherwise
     */
     ...
     * @throws AutoBuilderException
     *                              if the AutoBuilder has not been configured
    ...
    /**
     * Build a command to pathfind to a given position with rotation function.
     *
     * @param position
     *                         The position to pathfind to
     * @param rotationFunction
     *                         The function calculating the desired rotation given
     *                         the current
     *                         state of the trajectory
     * @param constraints
     *                         The constraints to use while pathfinding
     * @param goalEndVelocity
     *                         The goal end velocity of the robot when reaching the
     *                         target pose
    ...
    public static Command pathfindToPose(
        ...
        return pathfindToPoseFunctionalRotationCommandBuilder.apply(
                position, rotationFunction, constraints, goalEndVelocity);
    }
     ...
     *                              The constraints to use while pathfinding
     * @param goalEndVelocity
     *                              The goal end velocity of the robot when reaching
     *                              the target pose
     * @param rotationDelayDistance
     *                              The distance the robot should move from the
     ...
     *
     * @param pose
     *                              The pose to pathfind to. This will be flipped if
     *                              the path flipping
     *                              supplier returns
     *                              true
     * @param constraints
     ...
     *
     * @param pose
     *                        The pose to pathfind to. This will be flipped if the
     *                        path flipping
     *                        supplier returns
     *                        true
     * @param constraints
     ...
     *
     * @param pose
     *                    The pose to pathfind to. This will be flipped if the path
     *                    flipping
     *                    supplier returns
     *                    true
     * @param constraints
     ...
     *
     * @param goalPath
     *                               The path to pathfind to, then follow
     * @param pathfindingConstraints
     *                               The constraints to use while pathfinding
     * @param rotationDelayDistance
     *                               The distance the robot should move from the
     *                               start position before
     ...
     * project
     *
     * @param defaultAutoName
     *                        The name of the auto that should be the default
     *                        option. If this is
     *                        an
    ...
    /**
     * Get the starting pose from its JSON representation. This is only used
     * internally.
     *
     * @param startingPoseJson
     *                         JSON object representing a starting pose.
     ...
     *
     * @param autoJson
     *                 the JSON object to build the command from
